...
from cards import DashboardCard
...
def generate_dashboard(
    ...
    """Render dist/index.html from typed dashboard cards. Returns output path."""
    DIST_DIR.mkdir(parents=True, exist_ok=True)
    env = Environment(loader=FileSystemLoader(TEMPLATES_DIR), autoescape=True)
    template = env.get_template("dashboard.html")

    now_eastern = datetime.now(ZoneInfo("America/Toronto")).strftime("%Y-%m-%d %H:%M %Z")
    card_dicts = [card.to_template_dict() for card in cards]
